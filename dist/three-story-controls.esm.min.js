...
class CameraRig extends EventDispatcher {
  ...
  setAnimationPercentage(percentage) {
    if (this._hasAnimation) {
      const time = Math.max(0, Math.min(percentage * this.animationClip.duration, this.animationClip.duration - 1e-4));
...
const applyPiecewiseEasing = (t, segments) => {
...
class ScrollControls {
  constructor(cameraRig, props) {
    this.enabled = false;
    this.cameraStartPx = 0;
    this.cameraEndPx = 0;
    this.cameraBufferedStartPx = 0;
    this.cameraBufferedEndPx = 0;
    this.cameraRig = cameraRig;
...
const injectScrollAnimationStyles = () => {
...
class ScrollStoryControls extends EventDispatcher {
  constructor(cameraRig, props) {
    super();
...
function drawEasingCurve(canvas, easeFn) {
...
class CameraHelper {
  constructor(rig, controls, canvas, canvasParent) {
    ...
    this.scrollEase = Easings.linear;
    this.scrollEaseName = "linear";
    this.rig = rig;
  ...
  exportStoryConfig() {
    ...
    const config = {
      ...
      scroll: {
        ...
        startOffset: "-50vh",
        endOffset: "-50vh",
        ease: this.scrollEaseName !== "linear" ? this.scrollEaseName : void 0
      },
      transitions: {
  ...
  initUI(canvasParent) {
    ...
    speedValueLabel.textContent = `${this.scrollAutoPlayDuration / 1e3}s`;
    speedSlider.oninput = () => {
...
function resolveEasing(name) {
  if (!name || name === "linear") return void 0;
  return Easings[name];
}
async function createScrollStory(config, camera, scene, canvasElement, scrollElement, cameraData) {
  ...
  const controls = new ScrollStoryControls(rig, {
    ...
    canvasElement,
    ease: resolveEasing((_h = config.scroll) == null ? void 0 : _h.ease)
  });
  return { controls, rig };
}
